...
有人会问：把 provider 当参数传给策略不就行了？不行——
策略函数是**被引擎回调驱动的**：引擎逐 bar 调用它，调用签名是引擎定的
（本 demo 里 `my_strategy()` 无参；真实 ntrade 里 single 模式同样无参，
lifecycle 模式只收 context）。引擎不知道也不关心你的策略内部要什么依赖。
函数体内拿不到注入对象 → 依赖只能走"模块级函数 + 全局上下文"这条路。
所以全局上下文不是模式炫技，是"无参回调"约束下的必然选择。
