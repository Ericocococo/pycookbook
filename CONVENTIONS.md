...
14. **新建方案/设计类文档时，文件名 = `YYYYMMDD_NN_内容摘要.md`**（日期在前 + 序号 + 简要内容），
    如 `00_design/20260908_01_架构拆解与重构方案.md`；不叫泛化的 `design.md` / `plan.md`。
    同目录后续文档序号递增（02_、03_…）；文档内修订记录用表格 `| 序号 | 时间 | 简要内容 |` 逐行追加。
