...
parquet 自带 schema（数据结构定义：列名 + 类型），写入时可显式指定；
...
def demo01_explicit_schema():
    """① 显式指定 schema（string / int32 / float64 / timestamp），精确控制类型"""
    schema = pa.schema([
        ("symbol", pa.string()),
        ("volume", pa.int32()),     # int32：32 位有符号整数，范围 -2^31 ~ 2^31-1
        ("close",  pa.float64()),   # float64：64 位浮点数（双精度），即 Python float
        # timestamp("us")：时间戳类型，us = microseconds（微秒）精度
        ("ts", pa.timestamp("us")),
    ])
    table = pa.table({
        "symbol": ["AAPL", "MSFT"],
...
if __name__ == "__main__":
    ...
    demo01_explicit_schema()
    demo02_inspect_schema()