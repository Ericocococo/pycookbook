...
- **下半区(07–11,"用 Python 做什么")**(无限增长):`backend / data / ai / ops / projects`
...
└── 11_projects/                          # ═══ ★ 项目实战(端到端完整项目)═══
    # 每个子目录是一个可独立运行的完整项目,多文件协作、工程串联
    # 与 01–10 的单文件配方互补:配方学零件,项目学组装
```
...
| `11_projects` | 无限增长 | 项目实战: 端到端完整项目,多文件协作,配方学零件、项目学组装 |
...
4. **所有层级目录统一带两位序号前缀** `01_ 02_ ...` —— 顺序即导航;插入新目录就近取号,幅度大就整体重排。工程约定目录(`.github/`)按惯例不加号。
