"""
Pyright LSP WebSocket 桥接服务 —— websockets 同步版

对比异步版 (02_lsp_bridge_ws.py):
  - 不需要 asyncio / async / await
  - 用 threading 替代 asyncio.gather 实现双向读写
  - 启动: websockets.sync.server.serve() 替代 websockets.serve()

运行方式（在项目根目录）：
  python 10_ops/09_pyright/04_monaco_lsp/02_lsp_bridge_sync.py --serve
...
# WebSocket 处理（同步版）
# ──────────────────────────────────────────
def lsp_handler(ws) -> None:
    """
    每个连接启动一个 pyright 进程，
    两个子线程分别处理 ws→pyright 和 pyright→ws 方向。
    """
    ...
    def ws_to_pyright():
        """前端 → Pyright（同步读 ws，写 pyright stdin）"""
        try:
            for message in ws:                          # sync for 遍历 websocket
                pyright.stdin.write(message.encode())   # sync 写管道
                pyright.stdin.flush()
        except Exception:
            pass

    def pyright_to_ws():
        """Pyright → 前端（同步读 stdout，拦截注入后写 ws）"""
        while True:
            ...
            ws.send(json.dumps(data, ensure_ascii=False))  # sync 写 websocket
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pyright LSP WebSocket 桥接服务（同步版）")
    ...
    if args.serve:
        print(f"WebSocket 服务（同步版）:  ws://{args.host}:{args.port}")
        with serve(lsp_handler, args.host, args.port) as server:
            server.serve_forever()
    else:
        print("Pyright LSP WebSocket 桥接服务（websockets 同步版）")
        print()
        print("启动：python 02_lsp_bridge_sync.py --serve")
