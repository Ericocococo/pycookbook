...
def cmd_add(args: argparse.Namespace) -> str:
    """add 子命令的处理函数
    Namespace（命名空间）：argparse 存储解析结果的对象，用属性访问各参数值。
    """
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # dest（destination，目标属性名）：记录用户选了哪个子命令，存入 args.cmd
    # required=True：强制必须给一个子命令
    subparsers = parser.add_subparsers(dest="cmd", required=True)

    # 子命令 1: add
    p_add = subparsers.add_parser("add", help="添加一项")
    p_add.add_argument("name")
    p_add.set_defaults(func=cmd_add)    # 绑定处理函数，解析后 args.func 即 cmd_add
